...
const _require = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href)));
...
function getHash(text) {
  return crypto__default.hash("sha256", text, "hex").substring(0, 8);
}
...
function vuePlugin(rawOptions = {}) {
  ...
  const include = vue.shallowRef(
    rawOptions.include ?? /\.vue$/
  );
  const exclude = vue.shallowRef(rawOptions.exclude);
  let optionsHookIsCalled = false;
  const filter = vue.computed(() => vite.createFilter(include.value, exclude.value));
...
const distDir$1 = pathe.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))));
...
const packageDir = pathe.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..");
const distDir = pathe.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..", "dist");
...
const previewAnnotations = async (entry = []) => {
  const resolver = createResolver((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href)));
  return [
    ...entry.map((entry2) => {
...
const viteFinal = async (config, options) => {
  ...
  if (options.outputDir != null) {
    ...
    fs.writeFileSync(
      ...
      stringify__default(storybookViteConfig, { space: "  ", cycles: true }) || ""
    );
    fs.writeFileSync(
      path.join(options.outputDir, "logs", "vite-nuxt.config.json"),
      stringify__default(nuxtConfig, { space: "  ", cycles: true }) || ""
    );
    fs.writeFileSync(
      path.join(options.outputDir, "logs", "vite-final.config.json"),
      stringify__default(finalViteConfig, { space: "  ", cycles: true }) || ""
    );
  }
...
async function getPackageDir(packageName) {
  try {
    const require$1 = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href)));
